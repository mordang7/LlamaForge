...
def update_tray_menu():
    if tray_icon:
        ...
        tray_icon.title = (
            f"Llama.cpp GUI: {'Running' if service_running else 'Stopped'}"
        )
...
def create_icon():
    try:
        img = Image.open("icons/LlamaGUI_32.png")
        return img
...
def setup_tray():
    global tray_icon
    icon = create_icon()
    tray_icon = pystray.Icon("LlamaGUI", icon, "Llama.cpp GUI: Stopped", get_menu())
    tray_icon.run()
